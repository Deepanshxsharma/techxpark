...
class SlotLayoutHelper {
  ...
  Rect slotRect(int index) {
    ...
    final totalLaneAreaH = ParkingMapConfig.mapH - ParkingMapConfig.wallPad * 2 - rowAreaH * 4 - ParkingMapConfig.laneH * 2;
    ...
    switch (row) {
      case 0: // Top row — slots face down
        x = ParkingMapConfig.wallPad + col * (ParkingMapConfig.slotW + ParkingMapConfig.gapBetweenSlots);
        y = ParkingMapConfig.topOffset;
        break;
      case 1: // Upper-middle row — slots face up
        x = ParkingMapConfig.wallPad + col * (ParkingMapConfig.slotW + ParkingMapConfig.gapBetweenSlots);
        y = ParkingMapConfig.topOffset + rowAreaH + ParkingMapConfig.laneH + extraPad;
        break;
      case 2: // Lower-middle row — slots face down
        x = ParkingMapConfig.wallPad + col * (ParkingMapConfig.slotW + ParkingMapConfig.gapBetweenSlots);
        y = ParkingMapConfig.topOffset + (rowAreaH + ParkingMapConfig.laneH + extraPad) * 2;
        break;
      default: // Bottom row — slots face up
        x = ParkingMapConfig.wallPad + col * (ParkingMapConfig.slotW + ParkingMapConfig.gapBetweenSlots);
        y = ParkingMapConfig.topOffset + (rowAreaH + ParkingMapConfig.laneH + extraPad) * 3;
        break;
    }

    // Clamp within bounds
    x = x.clamp(ParkingMapConfig.wallPad, ParkingMapConfig.mapW - ParkingMapConfig.wallPad - ParkingMapConfig.slotW);
    y = y.clamp(ParkingMapConfig.topOffset, ParkingMapConfig.mapH - ParkingMapConfig.wallPad - ParkingMapConfig.slotH);

    final rect = Rect.fromLTWH(x, y, ParkingMapConfig.slotW, ParkingMapConfig.slotH);
    _slotCache[index] = rect; // Cache the physical layout coordinates
    return rect;
  }

  Offset entryPoint() => const Offset(ParkingMapConfig.mapW / 2, ParkingMapConfig.mapH - 8);
