  ...
  static void _drawVehicleVector(Canvas canvas, Offset pos, double angle) {
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        carRect.shift(const Offset(0, 4)),
    ...
    canvas.drawRect(
      Rect.fromCenter(center: const Offset(2, 0), width: 6, height: 10),
      glassPaint,
    );

    // Headlights (Beam effect)
    final lightPaint = Paint()
      ..shader = ui.Gradient.radial(const Offset(12, 0), 15, [
        Colors.yellow.withValues(alpha: 0.6),
        Colors.yellow.withValues(alpha: 0.0),
      ]);
    canvas.drawArc(
      Rect.fromCenter(center: const Offset(12, 0), width: 30, height: 40),
      -math.pi / 4,
      math.pi / 2,
      true,
      lightPaint,
